...
// Sends a branded welcome email to a newly provisioned client.
// Uses the Resend API directly so it works even before the user has accepted their invite.
Deno.serve(async (req) => {
  try {
    ...
    // Send email — NOTE: this only works after the user has accepted their invite and is registered.
    // Call this function after provisioning once the user exists in the app.
  ...
  <div style="padding:36px 32px;">
    ...
    <div style="background:#fef9c3;border:1px solid #fde047;border-radius:12px;padding:16px 20px;margin-bottom:24px;">
      <p style="margin:0;font-size:13px;color:#713f12;">
        <strong>📬 Note:</strong> You may also receive a separate automated email saying "You've been invited to join an app." That is a system notification — it does not mean you need to do anything extra. Your account is already fully set up. Simply use the instructions below to get started.
      </p>
    </div>
    ...
    <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:12px;padding:24px;margin-bottom:28px;">
      ...
      <div style="display:flex;gap:14px;margin-bottom:16px;align-items:flex-start;">
        ...
        <div>
          <p style="margin:0 0 4px 0;font-weight:600;color:#1e3a8a;font-size:14px;">Look for the "You're invited to join CatchACaller" email in your inbox</p>
